...
class MarketAPI {
    constructor() {
        this.config = {
            ...
            timeout: CONFIG.API.MARKET.TIMEOUT || 15000,
            retryCount: CONFIG.API.MARKET.RETRY_COUNT || 3,
        ...
        this.cache = {
            ...
            forecast: null,
            timestamp: null,
            ttl: CONFIG.API.MARKET.CACHE_TTL || 600000 // 10 minutes
        };
    }
    ...
    getApiKey() {
        if (typeof apiKeyManager !== 'undefined') {
            return apiKeyManager.getKey('market', false) || null;
        }
        
        const key = CONFIG.API.MARKET.KEY;
        if (!key || key === 'YOUR_API_KEY_HERE') {
            console.warn('[MarketAPI] No API key configured. Using sample data mode.');
            return null;
        }
        return key;
    }
    ...
    async getCurrentPrices(commodities = ['Wheat', 'Corn', 'Soybean']) {
        try {
            if (this.isCacheValid() && this.cache.prices) {
                console.log('[MarketAPI] Returning cached price data');
            ...
            if (apiKey) {
                ...
                return await this.fetchPricesFromAPI(commodities, apiKey);
            } else {
                // Use sample data mode
                console.log('[MarketAPI] Using sample price data (no API key)');
            ...
            console.error('[MarketAPI] Failed to fetch market prices:', error);
     ...
     * @private
     */
    async fetchPricesFromAPI(commodities, apiKey) {
        try {
            const url = `${this.config.baseUrl}${this.config.endpoints.PRICES}`;
            ...
            console.log('[MarketAPI] ✓ Price data fetched successfully');
            return data;
        } catch (error) {
            console.error('[MarketAPI] Error fetching from API:', error);
            throw error;
        }
    }
    ...
    async fetchWithRetry(url, retryCount = 0) {
        try {
            ...
            if (retryCount < this.config.retryCount && error.name !== 'AbortError') {
                console.log(`[MarketAPI] Retry ${retryCount + 1}/${this.config.retryCount}`);
